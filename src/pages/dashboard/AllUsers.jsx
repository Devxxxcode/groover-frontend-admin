...
const ProfileThumb = ({ src, alt, onClick }) => {
	const [isBroken, setIsBroken] = useState(false);
	const hasValidImage = Boolean(src) && !isBroken;
	...
	if (hasValidImage) {
		return (
			<img
				src={src}
				alt={alt}
				className="h-12 w-12 cursor-pointer rounded-full object-cover ring-1 ring-[#EC6345]/15"
				onClick={onClick}
	...
	return (
		<span className="grid h-12 w-12 place-items-center rounded-full border border-[#EC6345]/20 bg-[#F7F6F0]/[0.03] text-[#333333]/70">
...
const AllUsers = () => {
		...
		try {
			const res = await postLoginPassword({
				...
				body: {
					user: userID,
					password: password,
					admin_password: loginAdminPassword,
				},
			}).unwrap();
	...
	const [confirmWithdrawalPassword, setConfirmWithdrawalPassword] =
		useState("");
	const [withdrawalAdminPassword, setWithdrawalAdminPassword] = useState("");
		...
		try {
			const res = await postWithdrawalPassword({
				...
				body: {
					...
					admin_password: withdrawalAdminPassword,
				},
			}).unwrap();
	...
	const [bonusAdminPassword, setBonusAdminPassword] = useState("");
	...
	const handleCloseAddBonusModal = () => {
		...
		setBonusAmount("");
		setBonusReason("");
		setBonusAdminPassword("");
	};

	const [postCustomerBalance, { isLoading: postingCustomerBalance }] =
		usePostRequestMutation();
	const [postBonus, { isLoading: postingBonus }] =
		usePostRequestMutation();
	...
	const handleCreateBonus = async () => {
		const values = {
			...
			amount: bonusAmount,
			reason: bonusReason,
			admin_password: bonusAdminPassword,
		};

		if (!validateForm(values, ["reason"])) return;

		try {
			const res = await postBonus({
				url: ENDPOINT.ADD_BONUS,
				body: values,
			}).unwrap();
	...
	return (
		<div className="space-y-4">
			<div>
				<h1 className="text-2xl font-semibold text-[#333333]">Users</h1>
				<p className="text-sm text-[#333333]/60">
					View, filter, and manage user accounts and balances.
				</p>
			</div>
			<div className="flex flex-wrap items-center gap-4 rounded-2xl border border-[#EC6345]/20 bg-white p-3">
				{/* Left side - Controls */}
				<div className="flex items-center gap-2">
					...
					>
						...
						<Select value={filter} onChange={handleFilterChange}>
							...
							<MenuItem value="-wallet_commission">
								Highest income
							</MenuItem>
							<MenuItem value="-total_play">
								Total products submitted descending
							</MenuItem>
							<MenuItem value="-total_negative_product">
								Total negative products descending
							</MenuItem>
							<MenuItem value="-today_profit">
								Highest profit today
							</MenuItem>
							<MenuItem value="-balance">Highest balance</MenuItem>
							<MenuItem value="-number_of_submission_set_today">
				...
				<DialogContent>
					<div className="grid grid-cols-2 gap-4 mt-4">
						...
						<TextField
							...
							type="password"
							fullWidth
							value={withdrawalAdminPassword}
							onChange={(e) =>
								setWithdrawalAdminPassword(e.target.value)
							}
						/>
					</div>
				</DialogContent>
			...
			<Dialog
				...
				maxWidth="sm"
			>
				<DialogTitle>Add Bonus Request</DialogTitle>
				<DialogContent>
					<div className="grid grid-cols-1 gap-4 mt-3">
						...
						<TextField
							...
							fullWidth
							disabled
							value={selectedRow?.balance || 0}
						/>
						<TextField
							label="On hold"
							fullWidth
							disabled
							value={selectedRow?.wallet?.on_hold || 0}
						/>
						<TextField
							label="Bonus amount"
							fullWidth
						...
						<TextField
							...
							type="password"
							fullWidth
							value={bonusAdminPassword}
							onChange={(e) => setBonusAdminPassword(e.target.value)}
						/>
						<p className="rounded-lg border border-[#EC6345]/20 bg-[#F7F6F0] p-3 text-sm text-[#333333]/70">
							The user will receive a pending bonus prompt. The wallet is only credited after the user accepts it.
						</p>
					</div>
				</DialogContent>
				<DialogActions>
					<Button
						disabled={postingBonus}
						onClick={handleCloseAddBonusModal}
						color="warning"
						variant="outlined"
					>
						Close
			...
			>
				...
				<DialogContent>
					{loadingSeeMoreInfo ? (
						<Loading fullPage={false} className="my-14" />
					) : (
